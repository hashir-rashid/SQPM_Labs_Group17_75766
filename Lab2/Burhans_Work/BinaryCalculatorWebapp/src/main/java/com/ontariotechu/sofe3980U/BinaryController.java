...
public class BinaryController {
	...
	public String result(@RequestParam(name="operand1", required=false, defaultValue="") String operand1,
		...
		{
			case "+":
				model.addAttribute("result", Binary.add(number1,number2).getBinary());
				return "result";
			case "*":
				model.addAttribute("result", Binary.Multiply(number1,number2).getBinary());
				return "result";
			case "&":
				model.addAttribute("result", Binary.AND(number1,number2).getBinary());
				return "result";
			case "|":
				model.addAttribute("result", Binary.OR(number1,number2).getBinary());
				return "result";
			default:
